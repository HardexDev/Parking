...
void voiture(int arg){
    ...
    Attendre(5);
    printf("La Voiture %d est garer\n", (int)arg);
    pthread_cond_signal(&attendre);


    Attendre(rand()%3600);
    printf("La Voiture %d veut sortir du parking\n", (int)arg);
    usager.type = 1;
    if(msgsnd(msg_id, &usager, sizeof(Usager) - sizeof(long), 0) == -1) {
        perror("Erreur de envoie requete \n");
        exit(1);
    ...
    pthread_cond_signal(&attendre);
    printf("La Voiture %d est sortie du parking et part\n", (int)arg);
}
        ...
        if(response.type == 0){ //entrer
            if(response.estAbonne == 1){
                if(NB_PLACE_NON_ABO > 0){
                    NB_PLACE_NON_ABO--;
                    printf("\nNombre de places non abonnées : %d\nsignal SIGUSR1\n", NB_PLACE_NON_ABO);
